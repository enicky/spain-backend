...
exports.login = function login(request, identifier, password, next) {
  ...
  sails.log.verbose(__filename + ':' + __line )
  if (isEmail) {
    query.email = identifier;
  } else {
    query.username = identifier;
  }
  sails.log.verbose(__filename + ':' + __line + ' => Query : ', query);
  sails.models.user
    .findOne(query)
    .exec(function onExec(error, user) {
     // sails.log.verbose(__filename + ':' + __line + ' => user : ', user);
      if (error) {
        //sails.log.verbose(__filename + ':' + __line + ' => error : ', error);
        next(error);
      } else if (!user) {
        next(null, false);
      } else {
        sails.models.passport
          ...
          .exec(function onExec(error, passport) {
            //sails.log.verbose(__filename + ':' + __line + ' => error : ', error);
            //sails.log.verbose(__filename + ':' + __line + ' => passport : ', passport);
            
            if (passport) {
              passport.validatePassword(password, function callback(error, response) {
                 //sails.log.verbose(__filename + ':' + __line + ' => error : ', error);
              //sails.log.verbose(__filename + ':' + __line + ' => response : ', response);
